...
export const createTextToken = (command, index, text, originalNode) => {
...
export const substituteAlias = (command, token, alias) => {
...
export const expandCommand = (command, _cursorIndex, aliases) => {
...
export const getCommand = (buffer, aliases, cursorIndex) => {
...
export const getTopLevelCommands = (parseTree) => {
...
export const getAllCommandsWithAlias = (buffer, aliases) => {
    const parseTree = parse(buffer);
    const commands = getTopLevelCommands(parseTree);
    return commands.map((command) => expandCommand(command, command.tree.text.length, aliases));
};
