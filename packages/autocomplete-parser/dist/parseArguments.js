...
export const createArgState = (args) => {
    const updatedArgs = [];
...
export const flattenAnnotations = (annotations) => {
    const result = [];
...
export const optionsAreEqual = (a, b) => a.name.some((name) => b.name.includes(name));
export const countEqualOptions = (option, options) => options.reduce((count, opt) => (optionsAreEqual(option, opt) ? count + 1 : count), 0);
export const updateArgState = (argState) => {
...
export const getCurrentArg = (argState) => (argState.args && argState.args[argState.index]) || null;
export const isMandatoryOrVariadic = (arg) => !!arg && (arg.isVariadic || !arg.isOptional);
const preferOptionArg = (state) => isMandatoryOrVariadic(getCurrentArg(state.optionArgState)) ||
...
export const findOption = (spec, token) => {
...
export const findSubcommand = (spec, token) => {
...
const updateStateForOption = (state, token, isFinalToken = false) => {
    ...
    if (isRepeatable !== true && isRepeatable !== undefined) {
        ...
        if (currentRepetitions >= isRepeatable) {
            throw new UpdateStateError(`Cannot pass option again, already passed ${currentRepetitions} times, ` +
...
const updateStateForChainedOptionToken = (state, token, isFinalToken = false) => {
    ...
    if (isLongOption) {
        ...
        if (separatorMatches) {
            ...
            if ((optionState.optionArgState.args?.length ?? 0) > 1) {
                throw new UpdateStateError("Cannot pass argument with separator: option takes multiple args");
            }
    ...
    if (optionArg) {
        if ((optionState.optionArgState.args?.length ?? 0) > 1) {
            throw new UpdateStateError("Cannot chain option argument: option takes multiple args");
        }
...
const canConsumeSubcommands = (state) => !isMandatoryOrVariadic(getCurrentArg(state.optionArgState)) &&
...
const historyExecuteShellCommand = async () => {
    throw new ParsingHistoryError("Cannot run shell command while parsing history");
};
const getExecuteShellCommandFunction = (isParsingHistory = false) => isParsingHistory ? historyExecuteShellCommand : executeCommand;
const getGenerateSpecCacheKey = (completionObj, tokenArray) => {
    let cacheKey;
    ...
    if (generateSpecCacheKey) {
        if (typeof generateSpecCacheKey === "string") {
            ...
            logger.error("generateSpecCacheKey must be a string or function", generateSpecCacheKey);
        }
    }
...
const generateSpecForState = async (state, tokenArray, isParsingHistory = false, localLogger = logger) => {
    ...
    try {
        ...
        if (cacheKey && generateSpecCache.has(cacheKey)) {
            ...
            newSpec = convertSubcommand(await generateSpec(tokenArray, exec), initializeDefault);
            if (cacheKey)
                generateSpecCache.set(cacheKey, newSpec);
        }
        ...
        if (!(err instanceof ParsingHistoryError)) {
            localLogger.error(`There was an error with spec (generator owner: ${completionObj.name}, tokens: ${tokenArray.join(", ")}) generateSpec function`, err);
        }
    }
    return state;
};
...
export const initialParserState = getResultFromState(getInitialState({
    name: [""],
    subcommands: {},
    options: {},
    persistentOptions: {},
...
const parseArgumentsCache = createCache();
const parseArgumentsGenerateSpecCache = createCache();
const figCaches = new Set();
...
const getCacheKey = (tokenArray, context, specLocation) => [
...
const parseArgumentsCached = async (command, context, 
// authClient: AuthClient,
specLocations, isParsingHistory, startIndex = 0, localLogger = logger) => {
    const exec = getExecuteShellCommandFunction(isParsingHistory);
    ...
    for (let i = 0; i < locations.length; i += 1) {
        ...
        if (!isInDevMode() &&
            ...
            return (parseArgumentsGenerateSpecCache.get(cacheKey) ||
                parseArgumentsCache.get(cacheKey));
        }
    }
    let spec;
    let specPath;
    for (let i = 0; i < locations.length; i += 1) {
        ...
        spec = await withTimeout(5000, loadSubcommandCached(specPath, context, localLogger));
        if (!spec) {
            const path = specPath.type === SpecLocationSource.LOCAL ? specPath.path : "";
            localLogger.warn(`Failed to load spec ${specPath.name} from ${specPath.type} ${path}`);
        }
        else {
    ...
    let state = getInitialState(spec, tokens[0].text, specPath);
    let generatedSpec = false;
    const substitutedAliases = new Set();
    let aliasError;
    // Returns true if we should return state immediately after calling.
    const updateStateForLoadSpec = async (loadSpec, index, token) => {
        const loadSpecResult = typeof loadSpec === "function"
            ? token !== undefined
    ...
    for (let i = 1; i < tokens.length; i += 1) {
        if (state.completionObj.generateSpec) {
            state = await generateSpecForState(state, tokens.map((token) => token.text), isParsingHistory, localLogger);
            generatedSpec = true;
        }
        ...
        let loadSpec = lastType === TokenType.Subcommand
            ? state.completionObj.loadSpec
            : undefined;
        // Recurse for load spec or special arg
        if (lastType === lastArgType && lastArgObject) {
            const { isCommand, isModule, isScript, loadSpec: argLoadSpec, } = lastArgObject;
            if (argLoadSpec) {
                loadSpec = argLoadSpec;
            }
            else if (isCommand || isScript) {
                const specLocation = await getSpecPath(token, context.currentWorkingDirectory, Boolean(isScript));
...
export const parseArguments = async (command, context, 
// authClient: AuthClient,
isParsingHistory = false, localLogger = logger) => {
    const tokens = command?.tokens ?? [];
    if (!command || tokens.length === 0) {
        throw new ParseArgumentsError("Invalid token array");
    }
    if (tokens.length === 1) {
        const showFirstCommandCompletion = getSetting(SETTINGS.FIRST_COMMAND_COMPLETION);
    ...
    // authClient,
    undefined, isParsingHistory, 0, localLogger);
