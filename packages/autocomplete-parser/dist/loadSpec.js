 ...
 */
const searchFigFolder = async (currentDirectory) => {
    try {
        return ensureTrailingSlash((await executeCommand({
            command: "bash",
            args: [
                "-c",
...
export const getSpecPath = async (name, cwd, isScript) => {
    if (name === "?") {
    ...
    const personalShortcutsToken = getSetting(SETTINGS.PERSONAL_SHORTCUTS_TOKEN) || "+";
...
export const importSpecFromLocation = async (specLocation, localLogger = logger) => {
    // Try loading spec from `devCompletionsFolder` first.
    const devPath = isInDevMode()
        ? getSetting(SETTINGS.DEV_COMPLETIONS_FOLDER)
        : undefined;
    ...
    if (typeof devPort === "string" || typeof devPort === "number") {
        ...
        specFile = await importFromLocalhost(diffVersionedFile ? `${name}/${diffVersionedFile}` : name, devPort);
    }
    if (!specFile && devPath) {
        try {
            const { diffVersionedFile, name } = specLocation;
            const spec = await importSpecFromFile(diffVersionedFile ? `${name}/${diffVersionedFile}` : name, devPath, localLogger);
            specFile = spec;
        }
        catch {
    ...
    if (!specFile && specLocation.type === SpecLocationSource.LOCAL) {
        ...
        specFile = await importSpecFromFile(basename, `${dirname}.fig/autocomplete/build/`, localLogger);
    }
    else if (!specFile) {
        ...
        if (specFile) {
            ...
            try {
                specFile = await importSpecFromFile(name, `~/.fig/autocomplete/build/`, localLogger);
            }
            catch (_err) {
                /* empty */
            }
...
export const loadFigSubcommand = async (specLocation, _context, localLogger = logger) => {
...
export const loadSubcommandCached = async (specLocation, context, localLogger = logger) => {
    const { name, type: source } = specLocation;
    const path = specLocation.type === SpecLocationSource.LOCAL ? specLocation.path : "";
    // Do not load completion spec for commands that are 'disabled' by user
    const disabledSpecs = getSetting(SETTINGS.DISABLE_FOR_COMMANDS) || [];
